/-!
This is a live.lean-lang.org compatible version of the Session 2 demo.
It avoids Mathlib-only imports, notation, and tactics so it can run in the
browser playground. The downloadable Demo.lean file is the full course version.
-/
...
/-!
Optional classroom extension in the full course file uses Mathlib's
`Nat.factorization`. It is omitted here because live.lean does not load Mathlib.
-/
...
#check ({i : Nat // i < 3})
-- { i : Nat // i < 3 } : Type
#eval ((⟨2, by decide⟩ : {i : Nat // i < 3}).val)
-- 2
...
theorem twoSigma_ne_threeSigma : twoSigma ≠ threeSigma := by
  intro h
  have hfst : (2 : Nat) = 3 := congrArg Sigma.fst h
  nomatch hfst
...
theorem sixSubtype_ne_sevenSubtype : sixSubtype ≠ sevenSubtype := by
  intro h
  have hval : (6 : Nat) = 7 := congrArg Subtype.val h
  nomatch hval
...
#check List.rec
/-!
The full course file also checks Mathlib names `Nat.strongRec` and
`Nat.strong_induction_on`. They are omitted in this live.lean version.
-/
